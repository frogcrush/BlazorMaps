...
public partial class EventsPage
{
    ...
    private async Task HandleMouseEvent(MouseEvent mouseEvent) => await JsRuntime.InvokeVoidAsync("alert", $"Event type: {mouseEvent.Type} Lat: {mouseEvent.LatLng.Lat}, Lng: {mouseEvent.LatLng.Lng}");
    ...
    private async Task AddEventsToMap() => await mapRef.OnClick(async (MouseEvent mouseEvent) => await HandleMouseEvent(mouseEvent));

    private async Task RemoveEventsFromMap() => await mapRef.Off("click");
}
